 ...
 *				Shortest Job First				     *
 *===========================================================================*/
void schedule_proccess_shortest_job_first(struct schedproc p, unsigned flags) {
    for (int i = 0; i < NR_PROCS; i++) {
        int pos = i;
        for (int j = i + 1; i < NR_PROCS; j++) {
            if (Processes[i]->p_cpu_time_left < Processes[j]->p_cpu_time_left)
                pos = j;
            struct proc temp = Processes[i];
            Processes[i] = Processes[pos];
            Processes[pos] = temp;
        }
        Processes[0]->smallest_cpu_time_left = Processes[0]->p_cpu_time_left;
